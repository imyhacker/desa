...
class Profil extends CI_Controller
{
    ...
    public function vm()
    {
        ...
        $profil = $this->profil_m;
        $data['menu'] = $menu->getAll();
        $data['vm'] = $profil->getVm();
        $data['title'] = "Profil";

        $this->load->view('_partials/header', $data);
        $this->load->view('_partials/sidebar', $data);
        $this->load->view('_partials/top-menu');
        $this->load->view('admin/profil/vm');
        $this->load->view('_partials/footer');
    }

    public function updateVm($id = null)
    {
        $menu = $this->menu_m;
        $profil = $this->profil_m;
        $rules = $profil->rules();
        ...
        if ($validation->run() == false) {
            $data['title'] = "Profil";
            $data['menu'] = $menu->getAll();
            $data['vm'] = $profil->getById($id);
            $this->load->view('_partials/header', $data);
            $this->load->view('_partials/sidebar', $data);
            $this->load->view('_partials/top-menu');
            $this->load->view('admin/profil/edit_vm');
            $this->load->view('_partials/footer');
        } else {
            $profil->updateVm();
            $this->session->set_flashdata('message', '<div class="alert alert-success" role="alert">Sejarah desa berhasil diupdate!</div>');
            redirect('admin/profil/vm');
        }
    }
